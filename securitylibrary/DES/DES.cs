...
namespace SecurityLibrary.DES
{
	...
	public class DES : CryptographicTechnique
	{
		...
		public string shift_left(string k, int shifts)
		{
			...
			{
				for (int j = 1; j < 28; j++)//28 is the bits #
				{
					s += k[j];
				}
				s += k[0];
		...
		public string encrypt_decrypt(string Text, List<string> keys)
		{
			...
			for (int i = 0; i < 16; i++)
			{
				...
				for (int j = 0; j < 8; j++)
				{
					int row = Convert.ToInt32(x[j * 6].ToString()+ x[j * 6 + 5].ToString(),2); //form base 2 to base 10
					int col = Convert.ToInt32(x[j * 6 + 1].ToString() +x[j * 6 + 2].ToString() +
						                        x[j * 6 + 3].ToString() + x[j * 6 + 4].ToString(),2);
					int val = sbox[j][row][col];
					op = op + Convert.ToString(val, 2).PadLeft(4, '0'); //convert the value to its binary representation of 4 bits
					
				}
				op = permute(op, per);
				string tmp = xor(op, left);

				left = right;
				right = tmp;
				
			}
			string combinedkey = right + left; //32-bit swap
		...
		public override string Encrypt(string plainText, string key)
		{
			...
			List<string> keys = new List<string>();//holds keys of the 16 rounds
