...
namespace SecurityLibrary
{
    public class PlayFair : ICryptographicTechnique<string, string>
    {
        ...
        public string Decrypt(string cipherText, string key)
        {
            ...
            {               
                ...
                if (firstchar_col == secondchar_col) //the 2 chars in the same column
                {
                    ...
                    Dfirstchar_row = (firstchar_row - 1 + 5) % 5;  //adding 5 to handle -ve index and % 5 to ensure that the index is within the range
        ...
        public string Encrypt(string plainText, string key)
        {
            ...
            //CREATING MATRIX
            char[,] Matrix = CreateMatrix(key); //key matrix
            ...
            for (int i = 0; i < plaintext.Length; i += 2) //takes 2 letters at atime
            {
                if (i == plaintext.Length - 1) // if it is the last iteration & the length is an odd number 
                {
                    plaintext = plaintext + 'x';
                }
                else if (plaintext[i] == plaintext[i + 1]) //if the 2 letters are similar
                {
                ...
                for (int row = 0; row < 5; row++)
                {
                    for (int col = 0; col < 5; col++)
                    {
                        if (plaintext[i] == Matrix[row, col]) //get row & col of the first letter
                        {
                            firstchar_row = row;
                            firstchar_col = col;
                        }
                        else if (plaintext[i + 1] == Matrix[row, col]) //get row & col of the second letter
                        {
                            secondchar_row = row;
                            secondchar_col = col;
                        }
                ...
                { //same row and the other char's col
                    Efirstchar_row = firstchar_row;
