...
namespace SecurityLibrary
{
    public class Monoalphabetic : ICryptographicTechnique<string, string>
    {
        public string Analyse(string plainText, string cipherText)
        {
            ...
            for (int i = 0; i < 26; i++) //loop on alphabetic 
            {
                for (int j = 0; j < plaintext.Length; j++)
                {
                    ...
                    if (plaintext[j] == alphabet[i])//map plain letter to cipher letter (in order) 
                    {
                        key[i] = ciphertext[j];                        
                        CipherLetters.Add(ciphertext[j]);//Add all letters used in cipher text
                        break;
                    }
                }
            }
            for (int i = 0; i < 26; i++) //loop on key array
            {
                if (key[i] == ' ')
                {
                    for (int j = 0; j < 26; j++) //loop on alphabetic
                    {
