...
namespace SecurityLibrary
{
    ...
    public class HillCipher : ICryptographicTechnique<string, string>, ICryptographicTechnique<List<int>, List<int>>
    {
        ...
        public Matrix<double> MinorCofactor(Matrix<double> keyMatrix, int b)
        {
            Matrix<double> matrix = DenseMatrix.Create(3, 3, 0); //Cofactor Matrix
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    double result = ( b *Math.Pow(-1, i + j) 
                        * ((keyMatrix[x, y] * keyMatrix[x1, y1] - keyMatrix[x, y1] * keyMatrix[x1, y])%26)) % 26;
        ...
        public List<int> Decrypt(List<int> cipherText, List<int> key)
        {
            ...
            //Get m
            int m = (int) Math.Sqrt(key.Count); // m is number of rows of cipher matrix // mxm key

            //Construct KeyMatrix and CipherMatrix

            Matrix<double> keyMatrix = DenseMatrix.OfColumnMajor(m, key.Count / m, keyD.AsEnumerable()); //create a matrix column by column
            //Same as  Matrix<double> keyMatrix = DenseMatrix.OfColumnMajor(m, m, keyD.AsEnumerable());
            Matrix<double> CipherMatrix = DenseMatrix.OfColumnMajor(m, cipherText.Count / m, cipherD.AsEnumerable());
            
            
            List<int> finalResult = new List<int>();//will hold P.T
            if (m == 3)
            {
                int b = det(keyMatrix); // it returns the inverse of the determinant
                if (b == -1) //keyMatrix has no inverse
                {
